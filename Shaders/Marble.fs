...
module Perlin =
    ...
    let randomGradients =
        let gradient i = gradients.[i%16]
        let random = Random(0)
        let shuffle x = random.Next()
        p
        |> Array.map gradient

    let private cornerNoise (P:float3) t corner =
        let gradient =
            let index = int3(P + corner)
            let permute x = p.[x]
            let a = permute(index.x)
            let b = permute(a + index.y)
            let c = permute(b + index.z)
            gradients.[c%16]
        let s = dot gradient (t-corner)
        s

    let noise(pos:float3) =
        let P = (floor pos) % 256.0f
        let t = pos - P
        let corner = cornerNoise P t
        // Find the random noise values at the 8 corners of the surrounding unit cube
        let left =  float4(corner(float3(0.0f,0.0f,0.0f)),
                           corner(float3(0.0f,0.0f,1.0f)),
                           corner(float3(0.0f,1.0f,0.0f)),
                           corner(float3(0.0f,1.0f,1.0f)))
        let right = float4(corner(float3(1.0f,0.0f,0.0f)),
                           corner(float3(1.0f,0.0f,1.0f)),
                           corner(float3(1.0f,1.0f,0.0f)),
                           corner(float3(1.0f,1.0f,1.0f)))
        // Now find the average noise at the given position by using 
        // 5th order interpolation across each dimension
        let fade = t * t * t * (t * (t * 6.0f - 15.0f) + 10.0f)
        // Think of the cube vertices as two quads, one at x=0 and one at x=1
        // Reduce the cube to a single quad by interpolating in x
        let topDown = lerp(left, right, fade.x)
        ...
        let frontBack = lerp(topDown.xy, topDown.zw, fade.y)
        // Then reduce the line section to a point by interpolating in z
        lerpf(frontBack.x, frontBack.y, fade.z)
